...
每轮候选定义及源码先提交，完成后提交逐股结果、汇总和原因分析。`results/roundNN-stocks.csv.gz` 是可解压的UTF-8 CSV，`roundNN-summary.json` 保存源码提交、输入哈希、参数和分组指标；`development-leaderboard.csv` 仅用于开发期比较。所有尝试均留档，不只保留赢家。生成的大体积净值和交易明细保存在本地 `outputs/`。
